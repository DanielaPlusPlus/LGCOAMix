...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=1000,
        ...
        self.relu = nn.ReLU(inplace=True)
        self.conv5 = conv(512 * block.expansion, 256 * block.expansion, kernel_size=1, stride=1, padding=0, output_padding=0, transposed=True)
        ...
        self.conv10 = conv(32, 32, stride=2, transposed=True)
        self.bn10 = nn.BatchNorm2d(32)

        self.SA = SelfAttention(input_size=32)
        self.SAP = self.SuperpixelAttentionPooling
        self.fc_local = nn.Linear(32, num_classes)
    ...
    def forward(self, x, local=False, superpixel_map=None, topN_local_ratio=None):
        ...
        if local:
            ...
            # print(x_up1.shape, x3.shape)  #torch.Size([32, 1024, 14, 14]) torch.Size([32, 1024, 14, 14])
            x_up2 = self.relu(self.bn6(self.conv6(x_up1 + x3)))
            # print(x_up2.shape, x2.shape)  #torch.Size([32, 512, 28, 28]) torch.Size([32, 512, 28, 28])
            x_up3 = self.relu(self.bn7(self.conv7(x_up2 + x2)))
            # print(x_up3.shape, x1.shape)  #torch.Size([32, 256, 56, 56]) torch.Size([32, 256, 56, 56])
            x_up4 = self.relu(self.bn8(self.conv8(x_up3 + x1)))
            # print(x_up4.shape, x0.shape)  #torch.Size([32, 64, 56, 56]) torch.Size([32, 64, 56, 56])
            x_up5 = self.relu(self.bn9(self.conv9(x_up4 + x0)))
            # print(x_up5.shape) #torch.Size([32, 32, 112, 112])
            x_up6 = self.bn10(self.conv10(x_up5))
            # print(x_up6.shape) #torch.Size([32, 32, 224, 224])

            SPAttention_batch, topN_SurPFeat_batch, topN_SurPFeat_idx_batch = self.SAP(x_up6, superpixel_map, topN_local_ratio)  # list(32*[n, 256])
            ...
            return logits, topN_SurPLocalCls_batch, topN_SurPFeat_idx_batch, SPAttention_batch,  x_up6
        else:
            return x4, logits
